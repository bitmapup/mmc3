...
class MobilityTrace(object):
    ...
    def __init__(self, timestamp, latitude, longitude, cellid=None, eventType=None,
    ...
    def distance(self, mobilityTrace):
        ...
        a = math.sin(dLat / 2) ** 2 + (math.sin(dLon / 2) ** 2) * math.cos(float(self.latitude) * PIov180) * math.cos(
            float(mobilityTrace.latitude) * PIov180)
    ...
    def distance_only_latlon(cls, latitude1, longitude1, latitude2, longitude2):
        ...
        a = math.sin(dLat / 2) ** 2 + (math.sin(dLon / 2) ** 2) * math.cos(float(latitude1) * PIov180) * math.cos(
            float(latitude2) * PIov180)
        divisor = math.sqrt(1 - a)
        if (divisor == 0):
            divisor = 1
...
if __name__ == "__main__":
    ...
    logging.info("medioid: '{0}' centroid: '{1}'".format(mtm, mtc))

    filtered = MobilityTrace.filterSpeed(mylist, 22)
    logging.info("filterd list: {0}".format(filtered))
    ...
    logging.info("{0}".format(dist_matrix))

    spatial_filtered = MobilityTrace.spatial_filter(mylist)
    logging.info("{0}".format(spatial_filtered, 10))

    unique_antennas = MobilityTrace.count_number_of_unique_antennas(mylist)
    logging.info("{0}".format(unique_antennas))

    logging.info("{0}".format((mylist2[1] in mylist)))
