...
class Mmc(object):
    ...
    def __init__(self, cluster, trailMobilityTrace, userid,
        ...
        self._daysArrayEnum = [Day.MON, Day.TUE, Day.WED, Day.THU, Day.FRI, Day.SAT, Day.SUN, Day.WEEKDAYS,
                               Day.WEEKENDS, Day.ALL]
                    ...
                    if (i != j):
                        ...
                        while (k < len(self._timeLabels)):
                            aux_tuple = [poi_labels[i], poi_labels[j], \
                                         aux_spatioTemporalLabels[k][0], self._timeLabels[k][1], self._timeLabels[k][2]]
        ...
        if len(self._dict_mt_trajectory) != 0:
            for key in self._dict_mt_trajectory:
                ...
                for t in self._dict_mt_trajectory[key]:
                    ...
                    trajectory += 1
                    for mt in t:
                        logging.info("{0},{1}".format(trajectory, mt.cellid))
        else:
            logging.info("Empty trajectories")
    ...
    def printTrajectoriesToMap(self):
        ...
        logging.info("Trajectories:\n")
        trajectory = 0
        logging.info("id_trajectory,cell_id,latitude,longitude,icon")
        if len(self._dict_mt_trajectory) != 0:
            for key in self._dict_mt_trajectory:
                index_origin = key[0]
                ...
                if index_origin == index_destination:
                    ...
                    for t in self._dict_mt_trajectory[key]:
                        # print mobility traces of each trajecotry
                        trajectory += 1
                        logging.info("{0},{1},{2},{3},large_blue".format(
                            trajectory,
                            origin_mt.cellid,
                            origin_mt.latitude,
                            origin_mt.longitude))
                        ...
                        for mt in t:
                            logging.info("{0},{1},{2},{3},small_red".format(trajectory,
                                                                            mt.cellid,
                                                                            mt.latitude,
                                                                            mt.longitude))

                        logging.info("{0},{1},{2},{3},large_blue".format(trajectory,
    ...
    def __repr__(self):
        ...
        strResult += "\t Weekdays: Monday: {0}, Tuesday: {1}, Wednesday: {2}, Thursday: {3}, Friday: {4} \n".format(
            self._daysArrayBoolean[0], self._daysArrayBoolean[1], self._daysArrayBoolean[2], self._daysArrayBoolean[3],
            self._daysArrayBoolean[4])
        strResult += "\t Weekends: Saturday: {0}, sunday: {1}\n".format(self._daysArrayBoolean[5],
                                                                        self._daysArrayBoolean[6])
        strResult += "\t Weeks: Weekdays: {0}, Weekends: {1}, all: {2} \n".format(self._daysArrayBoolean[7],
                                                                                  self._daysArrayBoolean[8],
                                                                                  self._daysArrayBoolean[9])
    ...
    def vectorstationary(self):
        nueva_list = []
        for keys in self._cumulatedStationary:
            lista = [];
            aux_list = self._pois[keys]
            lista.append(self._cumulatedStationary[keys])
            lista.append(aux_list[1].latitude)
            lista.append(aux_list[1].longitude)
            nueva_list.append(lista)
    ...
    def averageperday(self):
        ...
        for keys in self._pois:
            aux_list = self._pois[keys]
            for i in aux_list[0]:
                oTime = (i.timestamp).weekday()
                iTime = (aux_list[1].timestamp).weekday()
                # print(oTime,i)
                if (oTime == 0):
                    mobilityMon.append(i)
                if (oTime == 1):
                    mobilityTue.append(i)
                if (oTime == 2):
                    mobilityWed.append(i)
                if (oTime == 3):
                    mobilityThu.append(i)
                if (oTime == 4):
                    mobilityFri.append(i)
                if (oTime == 5):
                    mobilitySat.append(i)
                if (oTime == 6):
                    mobilitySun.append(i)

        mobilityMon.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)
        mobilityTue.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)
        mobilityWed.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)
        mobilityThu.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)
        mobilityFri.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)
        mobilitySat.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)
        mobilitySun.sort(key=lambda MobilityTrace: MobilityTrace.timestamp)

        distanceMon, ptsMon = MobilityTrace.compute_cumulated_distance_day(mobilityMon)
        deviationMon = MobilityTrace.deviation(mobilityMon, distanceMon, ptsMon)
        distanceTue, ptsTue = MobilityTrace.compute_cumulated_distance_day(mobilityTue)
        deviationTue = MobilityTrace.deviation(mobilityTue, distanceTue, ptsTue)
        distanceWed, ptsWed = MobilityTrace.compute_cumulated_distance_day(mobilityWed)
        deviationWed = MobilityTrace.deviation(mobilityWed, distanceWed, ptsWed)
        distanceThu, ptsThu = MobilityTrace.compute_cumulated_distance_day(mobilityThu)
        deviationThu = MobilityTrace.deviation(mobilityThu, distanceThu, ptsThu)
        distanceFri, ptsFri = MobilityTrace.compute_cumulated_distance_day(mobilityFri)
        deviationFri = MobilityTrace.deviation(mobilityFri, distanceFri, ptsFri)
        distanceSat, ptsSat = MobilityTrace.compute_cumulated_distance_day(mobilitySat)
        deviationSat = MobilityTrace.deviation(mobilitySat, distanceSat, ptsSat)
        distanceSun, ptsSun = MobilityTrace.compute_cumulated_distance_day(mobilitySun)
        deviationSun = MobilityTrace.deviation(mobilitySun, distanceSun, ptsSun)

        averageperday = (
                                    distanceMon + distanceTue + distanceWed + distanceThu + distanceFri + distanceSat + distanceSun) / 7.0
        maxdeviation = max(deviationMon, deviationTue, deviationWed, deviationThu, deviationFri, deviationSat,
                           deviationSun)

        return averageperday, maxdeviation
    ...
    def mergeMmc(cls, aMmc, bMmc, threshold):
        if (aMmc.equals(bMmc)):
            ...
            newMatrixTransition, newMatrixLabels = cls.mergeTransMatrix(aMmc, bMmc, close_poiss_dict)
            # merge trajectories matrix
            # Test with new QTS will be on-line
            # /!\ new_trajectory = newTrajectories(aMmc,bMmc,close_poiss_dict)
            # Map clusters
            # change poi names
            new_pois = cls.newPois(aMmc, bMmc, close_poiss_dict)
    ...
    def newTrajectories(cls, aMmc, bMmc, close_pois_dict):
        ...
        for aKey in aTrajectory:
            aux_key = (str(aKey[0]) + 'a', str(aKey[1]) + 'a')
    ...
    def exportGraphviz_heatmap(self, path, value_vector):
        ...
        if (len(value_vector) == len(self._spatioTemporalLabels)):
            ...
            logging.info("WARNING value_vector is not of the same size of the nodes")
